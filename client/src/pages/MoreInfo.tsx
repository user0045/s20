...
const MoreInfo = () => {
  ...
  const getEpisodeCount = () => {
    if (content.type === 'series' || content.content_type === 'Web Series') {
      ...
      // Get episode count from show data - check multiple possible locations
      const episodeCount = content.show?.episode_id_list?.length || 
                          content.episode_id_list?.length ||
                          content.upload_content_data?.show?.episode_id_list?.length ||
                          0;
      return episodeCount > 0 ? `${episodeCount} Episode${episodeCount > 1 ? 's' : ''}` : null;
    }
    return null;
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6 min-w-0">
                  ...
                  <div className="space-y-4 border-t border-border/30 pt-6">
                    ...
                    {/* Directors */}
                    {(() => {
                      const directors = content.directors || content.director || 
                                      content.web_series?.seasons?.[0]?.director || 
                                      content.seasons?.[0]?.director ||
                                      content.movie?.director ||
                                      content.show?.directors ||
                                      content.upload_content_data?.movie?.director ||
                                      content.upload_content_data?.show?.directors;
                    ...
                    {/* Writers */}
                    {(() => {
                      const writers = content.writers || content.writer || 
                                    content.web_series?.seasons?.[0]?.writer || 
                                    content.seasons?.[0]?.writer ||
                                    content.movie?.writer ||
                                    content.show?.writers ||
                                    content.upload_content_data?.movie?.writer ||
                                    content.upload_content_data?.show?.writers;
                    ...
                    {/* Cast Members */}
                    {(() => {
                      const cast = content.cast || content.cast_members || 
                                 ...
                                 content.upload_content_data?.movie?.cast_members ||
                                 content.upload_content_data?.show?.cast_members;
